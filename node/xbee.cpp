      ...
      if (message[0] == 0x81)  // received message
      {
         ...
         switch(message[6])
         {
            ...
            case 0x28:{ // address assign
               ...
               m_neighbors[0].addr = wd(message[7],message[8]);
               m_neighbors[1].addr = wd(message[9],message[10]);
               break;
            }
            case 0x30:{ // deploy
               m_state = deploy;
   ...
   bool XBee::PingOut(u_char pid, u_int naddr)
   {
      ...
      msg[4] = frid; // frame id
      msg[5] = hb(m_neighbors[0].addr); // address high byte
      msg[6] = lb(m_neighbors[0].addr); // address low byte
      msg[8] = frid;
